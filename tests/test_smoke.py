...
# ---------- Camera barcode scanning (backend logic only, not wired to UI yet) ----------
# Founder: "can I scan barcode in to house of wax" -- decode_barcode_photo()
# is the core decode logic (pyzbar wraps the system ZBar library, present
# on Streamlit Cloud via packages.txt but not guaranteed locally -- see
# BARCODE_SCAN_AVAILABLE). Deliberately NOT wired into any widget yet
# (2026-09-08): the first attempt shipped the UI change in the same deploy
# as the new system dependency and crashed the live app -- see the
# house-of-wax-barcode-scan-deploy-incident memory. This deploy ships only
# the dependency + this dead-code function, to confirm that layer alone is
# safe before a follow-up adds st.camera_input.
...
def test_add_inventory_unaffected_by_dormant_barcode_scan_code():
    # This deploy adds the scan dependency + decode function but wires up
    # no UI yet -- Add Inventory must render exactly as it did before, with
    # no new widgets and no reference to camera scanning anywhere on the
    # page.
    import app as hw_app
    at = AppTest.from_file("app.py", default_timeout=30)
    at.session_state["testing_mode_enabled"] = True
    ...
    markdown_text = " ".join(m.value or "" for m in at.markdown)
    assert "Scan barcode with your camera" not in markdown_text, (
        "The camera scan UI should not be wired up in this deploy yet"
    )
