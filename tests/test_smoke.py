...
# core logic (pyzbar wraps the system ZBar library, present on Streamlit
# Cloud via packages.txt but not guaranteed locally -- see BARCODE_SCAN_
# AVAILABLE). st.camera_input() itself can't be driven through Streamlit's
# AppTest harness (same class of gap as st.file_uploader, already
# documented) -- these tests cover the decode function directly by
# monkeypatching the zbar decode call, plus a widget-level check that the
# page degrades gracefully when scanning isn't available (the real
# situation in this local/test environment, since libzbar0 isn't installed
# here). Real end-to-end scanning was verified by hand against the live
# deployed app instead.

def test_decode_barcode_photo_says_unavailable_when_zbar_missing(monkeypatch):
    import app as hw_app
...
def test_decode_barcode_photo_extracts_digits_from_a_zbar_result(monkeypatch):
    import app as hw_app
...
def test_decode_barcode_photo_prefers_a_real_length_barcode_over_stray_digits(monkeypatch):
    ...
    monkeypatch.setattr(hw_app, "BARCODE_SCAN_AVAILABLE", True)
    monkeypatch.setattr(hw_app, "_zbar_decode", lambda img: [_FakeDecoded(b"12345678901234567"), _FakeDecoded(b"012345678905")])
    monkeypatch.setattr(hw_app.Image, "open", lambda buf: object())
...
def test_decode_barcode_photo_gives_a_friendly_message_when_nothing_found(monkeypatch):
    import app as hw_app
    monkeypatch.setattr(hw_app, "BARCODE_SCAN_AVAILABLE", True)
    monkeypatch.setattr(hw_app, "_zbar_decode", lambda img: [])
    monkeypatch.setattr(hw_app.Image, "open", lambda buf: object())
...
def test_add_inventory_shows_scan_unavailable_message_when_zbar_missing():
    # In this test environment BARCODE_SCAN_AVAILABLE is False (no libzbar0
    # system library installed) -- the scan expander should say so plainly
    # instead of rendering a camera widget that can't work, and the
    # existing type-it-in fallback must still be reachable.
    import app as hw_app
    assert hw_app.BARCODE_SCAN_AVAILABLE is False, "This test assumes no system zbar library is installed locally"
    ...
    assert "scanning isn't set up in this environment" in info_text.lower(), (
        f"Expected the graceful scan-unavailable message, got info banners={info_text}"
    )
    text_input_keys = [t.key for t in at.text_input]
    assert "v24_lookup_barcode_primary_add_inventory" in text_input_keys, (
        "The manual barcode text field must still be reachable when scanning isn't available"
    )
